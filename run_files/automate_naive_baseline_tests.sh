...
do
  ...
  do
    ...
    then
      ...
      ./run_files/run_naive_init.sh $source_code $case_iterator 1 $runtime_json $netpaxos > "./logs/automated_logs/run_log_$iterator"_$case_iterator &
      #Wait so the test has time to finish
      wait
    else
      #Run the tests without initialization
      ./run_files/run_naive.sh $source_code $case_iterator 1 $runtime_json $netpaxos > "./logs/automated_logs/run_log_$iterator"_$case_iterator &
      #Wait so the test has time to finish
      wait
    fi
  ...
  python ./run_files/format_naive_results_to_csv.py
...
#After processing, create a backup the original log files
cd /home/apoorv/p4rl_tarantula/RL_for_P4/results/automated_results/
cp ./* /home/apoorv/p4rl_tarantula/RL_for_P4/results/backup/naive-tests/
rm ./*

echo "AUTOMATED TEST FOR NAIVE FUZZER HAS FINISHED"
