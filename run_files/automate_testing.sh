...
do
  ...
  do
    ...
    then
      ...
      ./run_files/run_with_initialization.sh $source_code $case_iterator 1 $runtime_json $netpaxos $random_act > "./logs/automated_logs/run_log_$iterator"_$case_iterator &
      #Wait so the test has time to finish
      wait
    else
      #Run the tests without initialization
      ./run_files/run_normal.sh $source_code $case_iterator 1 $runtime_json $netpaxos $random_act > "./logs/automated_logs/run_log_$iterator"_$case_iterator &
      #Wait so the test has time to finish
      wait
    fi
...
#After processing, create a backup version of the original log files
cd /home/apoorv/p4rl_tarantula/RL_for_P4/results/automated_results/
cp ./* /home/apoorv/p4rl_tarantula/RL_for_P4/results/backup

echo "AUTOMATED TEST FOR P6 HAS FINISHED"
